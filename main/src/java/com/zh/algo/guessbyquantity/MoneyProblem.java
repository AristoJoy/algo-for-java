...
public class MoneyProblem {
    ...
    private static long process(int[] d, int[] p, int ability, int index) {
        ...
        if (ability < d[index]) {
            return p[index] + process(d, p, d[index] + ability, index + 1);
        } else {
            return Math.min(p[index] + process(d, p, d[index] + ability, index + 1), process(d, p, ability, index + 1));
        }
    }
