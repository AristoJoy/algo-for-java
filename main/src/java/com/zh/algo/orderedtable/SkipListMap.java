...
public class SkipListMap<K extends Comparable<K>, V> {
    ...
    public void put(K key, V value) {
        ...
        if (find != null && find.isKeyEqual(key)) {
            ...
            SkipListNode<K, V> newSkipListNode = new SkipListNode<>(key, value);
            for (int i = 0; i <= newNodeLevel; i++) {
                newSkipListNode.nextNodes.add(null);
            }
            ...
            while (level >= 0) {
                ...
                if (level <= newNodeLevel) {
                    newSkipListNode.nextNodes.set(level, pre.nextNodes.get(level));
                    pre.nextNodes.set(level, newSkipListNode);
                }
                level--;
            }
        }
    ...
    static class SkipListNode<K extends Comparable<K>, V> {
        ...
        public boolean isKeyEqual(K otherKey) {
            return (key == null && otherKey == null) || (otherKey != null && key != null && key.compareTo(otherKey) == 0);
        }
    }
